...
STEPS = [16, 12, 8, 4, 0]  # quarterly offsets
HORIZON_Q = 4  # predict 4 quarters ahead

## Map momentum features
momentum_mapping = {"1 month": "mom_1m", "3 months": "mom_3m", "6 months": "mom_6m", "9 months": "mom_9m"}
MOMENTUM_COLS = [momentum_mapping[period] for period in config["momentum_features"] if period in momentum_mapping]

FUNDAMENTAL_COLS = ["Revenue","COGS","EBIT","SGA","NetIncome","Cash","Receivables","Inventories","OtherCurrentAssets",
                   "PPE","OtherAssets","DebtCurrent","AccountsPayable","TaxesPayable","LiabilitiesCurrent","TotalLiabilities"]
...
mlp_predictions_file = get_output_path("predictions_mlp.csv", session_id)
save_predictions_csv(mlp_predictions_file,preds,y,t_idx,tics)

mlp_metrics_file = get_output_path("metrics_mlp.txt", session_id)
...
lstm_predictions_file = get_output_path("predictions_lstm.csv", session_id)
save_predictions_csv(lstm_predictions_file,preds,y,t_idx,tics)

lstm_metrics_file = get_output_path("metrics_lstm.txt", session_id)
